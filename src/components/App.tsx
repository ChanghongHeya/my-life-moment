...
function App() {
  ...
  const isLifeProgressEvent = (event: CountdownEvent) =>
    event.id === 'default-2' || event.title === '人生进度';
  ...
  const handleCloseLifeProgressDetail = () => {
    setLifeProgressDetailEvent(null);
  };

  const handleEditLifeProgress = () => {
    if (!lifeProgressDetailEvent) return;
        ...
        {lifeProgressDetailEvent && (
          <>
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              onClick={handleCloseLifeProgressDetail}
                ...
                <div className="glass-card rounded-3xl p-4 shadow-2xl">
                  <div className="flex items-center justify-between gap-3 mb-4">
                    <div>
                      ...
                      <p className="text-xs text-gray-500 dark:text-gray-400">点击后查看百分比与阶段信息</p>
                    </div>
                    <button
                      onClick={handleCloseLifeProgressDetail}
